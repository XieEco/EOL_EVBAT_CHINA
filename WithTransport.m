...
Capcity_step3_SYL=1500./4545;
Capcity_step3_LSTL=1500./6667;
for j=1:31
...
Capcity_step4_ECLY_LSTL=2000./6667;
for j=1:31
...
%%%%%%%%%%%%%  Optimal (open this section)  %%%%%%%%%%%%
%load ('MTKL.mat','lowest_cost','YH_WSJL','YH_CNL','YH_XHL','YH_sudu','WSJL_SUDU')
%tdqj=lowest_cost;%tdqj=lowest_co2;
%WSJL_sheng(:,1)=YH_WSJL(:,tdqj);
%CNL_sheng(:,1)=YH_CNL(:,tdqj);
%XHL_sheng(:,1)=YH_XHL(:,tdqj);
%CNL_nian(:,1)=CNL_sheng(:,1)./30;
%XHL_nian(:,1)=XHL_sheng(:,1)./30;
%for j=1:21 %2020 to 2040
%WSJL(:,j)=0.75-(0.75-WSJL_sheng(:,1)).*WSJL_SUDU(YH_sudu(:,tdqj),j);
%end
%for j=22:31
%WSJL(:,j)=WSJL_sheng(:,1);
%end
...
%%%%%%%%%%%%%  Senarios  (open this section,choose one Senario)  %%%%%%%%%%%%
...
qjs=13;%HIG-B5
for i=1:31
WSJLT(i,:)=YSQJCS(4+(qjs-1).*4,:);
CNLT(i,:)=YSQJCS(2+(qjs-1).*4,:);
XHLT(i,:)=YSQJCS(3+(qjs-1).*4,:);
YJLT(i,:)=YSQJCS(1+(qjs-1).*4,:);
end
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

Yunshuliang_SYL_2035=zeros(31,31);
...
%Cost_total(qjs,:)=sum(Cost_total_province);
%CO2_total(qjs,:)=sum(CO2_total_province);
    for j=1:31
        %Cost
    Cost_step1_total(:,j)=Cost_step1_total(:,j);
    Cost_step2_total(:,j)=Cost_step2_total(:,j);
    Cost_step3_total(:,j)=Cost_step3_total(:,j);    
    Cost_step4_Yejin_total(:,j)=Cost_step4_total_YEJIN(:,j);
    Cost_step4_TCLY_total(:,j)=Cost_step4_total_CHUNENG(:,j);
    Cost_step4_ZJHS_total(:,j)=Cost_step4_total_XUNHUAN(:,j);
    Cost_step5_TCLY_total(:,j)=Cost_step5_LSTL_CN(:,j);
    Cost_step5_XUNHUAN_total(:,j)=Cost_step5_LSTL_XH(:,j)+Cost_step5_SYL_XH(:,j);
    %CO2
    CO2_step1_total(:,j)=CO2_step1_total(:,j);
    CO2_step2_total(:,j)=CO2_step2_total(:,j);
    CO2_step3_total(:,j)=CO2_step3_total(:,j);    
    CO2_step4_Yejin_total(:,j)=CO2_step4_total_YEJIN(:,j);
    CO2_step4_TCLY_total(:,j)=CO2_step4_total_CHUNENG(:,j);
    CO2_step4_ZJHS_total(:,j)=CO2_step4_total_XUNHUAN(:,j);
    CO2_step5_TCLY_total(:,j)=CO2_step5_LSTL_CN(:,j);
    CO2_step5_XUNHUAN_total(:,j)=CO2_step5_LSTL_XH(:,j)+CO2_step5_SYL_XH(:,j);
    end
    RESULT_CC=zeros(682,31);
    RESULT_CC(1:31,:)=WSJLT(:,:);
    RESULT_CC(32:62,:)=CNLT(:,:);
    RESULT_CC(63:93,:)=XHLT(:,:);
    RESULT_CC(94:124,:)=YJLT(:,:);
    RESULT_CC(125:155,:)=Cost_step1_total(:,:)./10^9;
    RESULT_CC(156:186,:)=Cost_step2_total(:,:)./10^9;
    RESULT_CC(187:217,:)=Cost_step3_total(:,:)./10^9;
    RESULT_CC(218:248,:)=Cost_step4_Yejin_total(:,:)./10^9;
    RESULT_CC(249:279,:)=Cost_step4_TCLY_total(:,:)./10^9;
    RESULT_CC(280:310,:)=Cost_step4_ZJHS_total(:,:)./10^9;
    RESULT_CC(311:341,:)=Cost_step5_TCLY_total(:,:)./10^9;
    RESULT_CC(342:372,:)=Cost_step5_XUNHUAN_total(:,:)./10^9;
    RESULT_CC(373:403,:)=CO2_step1_total(:,:)./10^6;
    RESULT_CC(404:434,:)=CO2_step2_total(:,:)./10^6;
    RESULT_CC(435:465,:)=CO2_step3_total(:,:)./10^6;
    RESULT_CC(466:496,:)=CO2_step4_Yejin_total(:,:)./10^6;
    RESULT_CC(497:527,:)=CO2_step4_TCLY_total(:,:)./10^6;
    RESULT_CC(528:558,:)=CO2_step4_ZJHS_total(:,:)./10^6;
    RESULT_CC(559:589,:)=CO2_step5_TCLY_total(:,:)./10^6;
    RESULT_CC(590:620,:)=CO2_step5_XUNHUAN_total(:,:)./10^6;
    RESULT_CC(621:651,:)=Cost_total_province(:,:)./10^9;
    RESULT_CC(652:682,:)=CO2_total_province(:,:)./10^6; 
save WithTranspot.mat
FFF=zeros(7,2);
FFF(1,1)=sum(sum(RESULT_CC(125:155,:),2),1);
FFF(2,1)=sum(sum(RESULT_CC(156:186,:),2),1);
FFF(3,1)=sum(sum(RESULT_CC(187:217,:),2),1);
FFF(4,1)=sum(sum(RESULT_CC(311:372,:),2),1);
FFF(5,1)=sum(sum(RESULT_CC(218:248,:),2),1);
FFF(6,1)=sum(sum(RESULT_CC(249:279,:),2),1);
FFF(7,1)=sum(sum(RESULT_CC(280:310,:),2),1);

FFF(1,2)=sum(sum(RESULT_CC(373:403,:),2),1);
FFF(2,2)=sum(sum(RESULT_CC(404:434,:),2),1);
FFF(3,2)=sum(sum(RESULT_CC(435:465,:),2),1);
FFF(4,2)=sum(sum(RESULT_CC(559:620,:),2),1);
FFF(5,2)=sum(sum(RESULT_CC(466:496,:),2),1);
FFF(6,2)=sum(sum(RESULT_CC(497:527,:),2),1);
FFF(7,2)=sum(sum(RESULT_CC(528:558,:),2),1);

FIG_OS= RESULT_CC(1:124,:);

Yunshuliang_CN_2050=Yunshuliang_LSTL_2050_CN;
Yunshuliang_CN_2035=Yunshuliang_LSTL_2035_CN;
Yunshuliang_XH_2035=Yunshuliang_LSTL_2035_XH+Yunshuliang_SYL_2035;
Yunshuliang_XH_2050=Yunshuliang_LSTL_2050_XH+Yunshuliang_SYL_2050;
sum(sum(Yunshuliang_CN_2050,2),1)
sum(sum(Yunshuliang_CN_2035,2),1)
sum(sum(Yunshuliang_XH_2035,2),1)
sum(sum(Yunshuliang_XH_2050,2),1)
GAP_CN(1:31,:)=GG_YH_CN(32:62,:)-XQ_TCLY(32:62,:);
GAP_XH_SYL(1:31,:)=GG_YH_XH(1:31,:)-XQ_XH3B(1:31,:);  
GAP_XH_LSTL(1:31,:)=GG_YH_XH(32:62,:)-XQ_XH3B(32:62,:);
GAP_XH=GAP_XH_SYL+GAP_XH_LSTL;    
