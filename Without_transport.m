...
if       GAP_panduan(i,j)==0
    XQ_TCLY(i+31,j)=GG_YH_CN(i+31,j);
else
end
            end
        end
...
% Direct recycling
  GAP_XH_SYL(1:31,:)=GG_YH_XH(1:31,:)-XQ_XH3B(1:31,:);%循环三元锂GAP，GWh    
    GAP_XH_LSTL(1:31,:)=GG_YH_XH(32:62,:)-XQ_XH3B(32:62,:);%循环磷酸铁锂GAP
    GAP_panduan1=zeros(31,31);%判断矩阵
    GAP_panduan2=zeros(31,31);%判断矩阵    
    for i=1:31
    for j=1:31
    if GAP_XH_SYL(i,j)>=0
        GAP_panduan1(i,j)=0;
...
for i=1:31
    for j=1:31
        if GAP_panduan1(i,j)==0
             XQ_XH3B(i,j)=GG_YH_XH(i,j);
             XQ_XH3B(i+62,j)=GG_YH_XH(i+62,j);  
        else
        end
        if GAP_panduan2(i,j)==0
        XQ_XH3B(i+31,j)=GG_YH_XH(i+31,j);    
         else
        end
        end
end
...
%CO2_total(qjs,:)=sum(CO2_total_province);
    ...
    Cost_step5_TCLY_total(:,j)=0;
    Cost_step5_XUNHUAN_total(:,j)=0;
    %CO2
    ...
    CO2_step5_TCLY_total(:,j)=0;
    CO2_step5_XUNHUAN_total(:,j)=0;
    end
