...
%Power_pv=csvread('pv.csv').*0.01.*1.1;
%Power_pv=csvread('pv.csv').*0.01.*0.9;
%Power_wind=csvread('wind.csv').*0.01;
%Power_wind=csvread('wind.csv').*0.01.*1.1;
Power_wind=csvread('wind.csv').*0.01.*0.9;
CF=csvread('CF.csv');
for i=1:31
...
%shengjianyunju=csvread('SHENGJIYUNSHU.csv').*1.2;
%shengjianyunju=csvread('SHENGJIYUNSHU.csv').*1.5;
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%%% Fig 2
for i=1:12
FIG2_COST(i,:)=(Cost_total(i+1,:)-Cost_total(1,:))./10^9;%B CNY
FIG2_CO2(i,:)=(CO2_total(i+1,:)-CO2_total(1,:))./10^6;%Mt
end
LJZ_co2=sum(CO2_total(:,:),2)./10^6;%Mt;
LJZ_cost=sum(Cost_total(:,:),2)./10^9;%B CNY
ccjgz=zeros(12,2);
for i=1:12
ccjgz(i,1)=LJZ_cost(i+1,1)-LJZ_cost(1,1);
ccjgz(i,2)=LJZ_co2(i+1,1)-LJZ_co2(1,1);
end
costa=LJZ_cost(1,1);
co2a=LJZ_co2(1,1);
...
%load UA_r_1.5.mat
%load UA_r_3.mat
%load UA_fakuan_4.mat
%load UA_fakuan_5.mat
%load UA_capacity_10.mat
%load UA_capacity_20.mat
%load UA_capacity_30.mat
%load UA_transport_20.mat
%load UA_transport_50.mat
%load UA_pv_10.mat
%load UA_pv_f10.mat
%load UA_wind_10.mat
load UA_wind_f10.mat
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
b=kkk;
YH_lowest_cost=min(YH_result_COST);
YH_lowest_co2=min(YH_result_CO2);
YH_higest_cost=max(YH_result_COST);
YH_higest_co2=max(YH_result_CO2);
YH_average_cost=mean(YH_result_COST);
YH_average_co2=mean(YH_result_CO2);
for j=1:b
if YH_result_COST(1,j)==YH_lowest_cost(1,1)
lowest_cost=j;
else
end
if YH_result_CO2(1,j)==YH_lowest_co2(1,1)
lowest_co2=j;
else
end
if YH_result_COST(1,j)==YH_higest_cost(1,1)
higest_cost=j;
else
end
if YH_result_CO2(1,j)==YH_higest_co2(1,1)
higest_co2=j;
else
end
end
...
Cost_UA=sum(sum(RESULT_CC(621:651,:),2),1)-costa(1,1)
CO2_UA=sum(sum(RESULT_CC(652:682,:),2),1)-co2a(1,1)